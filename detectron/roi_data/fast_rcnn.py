...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training and cfg.MODEL.MASK_ON:
        ...
        # blob_names += ['mask_labels']
        # blob_names += ['mask_weights']
        if cfg.MRCNN.USE_CLS_EMBS:
            blob_names += ['fg_mask']
            blob_names += ['bg_mask']
            blob_names += ['fg_norm']
            blob_names += ['bg_norm']
            blob_names += ['mask_emb_fg_labels']
            blob_names += ['mask_emb_bg_labels']
          #  blob_names += ['mask_emb_weights']
        if cfg.MRCNN.BBOX_CASCADE_MASK_ON:
            blob_names += ['inter_masks_int32']
    ...
    if is_training and cfg.MODEL.KEYPOINTS_ON:
        ...
        #    blob_names += ['inter_keypoint_int32']
    ...
    if is_training and cfg.MODEL.BODY_UV_ON:
        ...
        blob_names += ['body_mask_labels']

        # blob_names += ['body_uv_Index_targets']
        # blob_names += ['body_uv_Index_targets_weights']
        # blob_names += ['body_uv_U_targets']
        # blob_names += ['body_uv_V_targets']
        # blob_names += ['body_uv_weights']
        ...
        if cfg.BODY_UV_RCNN.USE_BOX_ALL_MASKS:
            blob_names += ['body_masks_wrt_box']
            # blob_names += ['semantic_segms']
