...
def convert_polys_to_mask_wrt_box(polygons, box):
    """Convert from the COCO polygon segmentation format to a binary mask
    encoded as a 2D array of data type numpy.float32. The polygon segmentation
    is understood to be enclosed in the given box and rasterized to an box_h x box_w
    mask. The resulting mask is therefore of shape (box_h, box_w).
    """
    w = box[2] - box[0]
    h = box[3] - box[1]
    ...
    rle = mask_util.frPyObjects(polygons_norm, h, w)
