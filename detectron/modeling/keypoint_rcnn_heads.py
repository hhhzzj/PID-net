...
def add_keypoint_outputs(model, blob_in, dim):
    ...
    # NxKxHxW
    if cfg.KRCNN.BBOX_CASCADE_KPT_ON:
        return add_cascaded_kpt_outputs(model, blob_in, dim)
...
def add_keypoint_losses(model):
    ...
    losses = ['loss_kps']
    losses_to_gradients = [loss_kps]
    
    if cfg.KRCNN.BBOX_CASCADE_KPT_ON:
        
        model.net.Reshape(
           ['inter_kps_score'], ['inter_kps_score_reshaped', '_inter_kps_score_old_shape'],
           shape=(-1, cfg.KRCNN.HEATMAP_SIZE * cfg.KRCNN.HEATMAP_SIZE)
        )
        inter_kps_prob, loss_inter_kps = model.net.SoftmaxWithLoss(
            ['inter_kps_score_reshaped', 'keypoint_locations_int32', 'keypoint_weights'],
            ['inter_kps_prob', 'loss_inter_kps'],
            scale= 0.1*cfg.KRCNN.LOSS_WEIGHT / cfg.NUM_GPUS,
            spatial=0
        )
        
       # loss_inter_kps = model.net.SigmoidCrossEntropyLoss(
       #     ['inter_kps_score', 'inter_keypoint_int32'],
       #     'loss_inter_kps',
       #     scale=model.GetLossScale() * cfg.KRCNN.LOSS_WEIGHT
       # )
        
        losses += ['loss_inter_kps']
        losses_to_gradients += [loss_inter_kps]
...
def add_roi_pose_head_v1convX(model, blob_in, dim_in, spatial_scale):
    ...
    if cfg.KRCNN.BBOX_CASCADE_KPT_ON:
        return add_roi_kpt_cascade_head(model, blob_in, dim_in, spatial_scale)
...
def add_roi_kpt_cascade_head(model, blob_in, dim_in, spatial_scale):
    ...
    for i in range(cfg.KRCNN.NUM_STACKED_CONVS // 2):
        current = model.Conv(
            current,
            '_[pose]_conv_fcn' + str(i + 1),
            dim_in,
        ...
        current = model.LeakyRelu(current, current)
        dim_in = hidden_dim
    # add intermediate out
    
    inter = model.ConvTranspose(current, 'Inter_head_out_lowres', hidden_dim, hidden_dim,
                                cfg.KRCNN.DECONV_KERNEL, pad=int(cfg.KRCNN.DECONV_KERNEL / 2 - 1),
                                stride=2, weight_init=(cfg.KRCNN.CONV_INIT, {'std': 0.001}),
                                bias_init=('ConstantFill', {'value': 0.}))
    inter = model.LeakyRelu(inter, inter)
    '''
    inter = model.BilinearInterpolation(
           inter, 'Inter_head_out_upres', hidden_dim,
           hidden_dim, cfg.KRCNN.UP_SCALE
    )
    
    inter = model.ConvTranspose(inter, 'Inter_head_out_upres', hidden_dim, hidden_dim, cfg.KRCNN.DECONV_KERNEL,
                                pad=int(cfg.KRCNN.DECONV_KERNEL / 2 - 1), stride=2,
                                weight_init=(cfg.KRCNN.CONV_INIT, {'std': 0.001}),
                                bias_init=('ConstantFill', {'value': 0.}))
    inter = model.LeakyRelu(inter, inter)
    '''
    ...
    model.BilinearInterpolation(
           'inter_kps_score_lowres', 'inter_kps_score', cfg.KRCNN.NUM_KEYPOINTS,
           cfg.KRCNN.NUM_KEYPOINTS, cfg.KRCNN.UP_SCALE
    )
    # add final head out
    for i in range(cfg.KRCNN.NUM_STACKED_CONVS // 2):
    ...
    current = model.ConvTranspose(current, 'kp_head_out_lowres', hidden_dim, hidden_dim,
                                  cfg.KRCNN.DECONV_KERNEL, pad=int(cfg.KRCNN.DECONV_KERNEL / 2 - 1),
                                  stride=2, weight_init=(cfg.KRCNN.CONV_INIT, {'std': 0.001}),
                                  bias_init=('ConstantFill', {'value': 0.}))
    current = model.LeakyRelu(current, current)
    
    #current = model.BilinearInterpolation(
    #       current, 'kp_head_out_upres', hidden_dim,
    #       hidden_dim, cfg.KRCNN.UP_SCALE
    #)

    '''
    current = model.ConvTranspose(current, 'kp_head_out_upres', hidden_dim, hidden_dim,
    ...
    current = model.Concat([current, inter], 'kp_head_out_concated', axis=1)
...
def add_cascaded_kpt_outputs(model, blob_in, dim):
