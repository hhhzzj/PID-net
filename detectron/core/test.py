...
def im_detect_all(model, im, box_proposals, timers=None):
    ...
    if cfg.MODEL.BODY_UV_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.UV_AUG.ENABLED:
            cls_bodys = im_detect_aug_body_uv(model, im, boxes)
        else:
...
def im_detect_mask(model, im_scale, boxes):
    ...
    #     embs = workspace.FetchBlob(
    #         core.ScopedName('mask_fcn_emb')
    #     )
    #     bs, dims = embs.shape[0], embs.shape[1]
    #     eps=1e-6
    #     for i in range(bs):
    #         p_m = (pred_masks[i,1]>0.5).astype(np.float32)
    #         if p_m.max()==0:
    #             continue
    #         ref_emb = np.sum(embs[i]*p_m[np.newaxis],axis=(1,2))/(np.sum(p_m)+eps)
    #         ref_emb = np.reshape(ref_emb,(1,dims))
    #         # tmp_fg_embs = embs[i][:,np.where(p_m>0)]
    #         emb = np.reshape(embs[i],(dims,-1)).transpose()
    #         tmp_fg_embs = emb[p_m.reshape((-1))>0]
    #         thr = (np.sum((tmp_fg_embs - ref_emb)**2,axis=1)).mean()
    #         sims = np.sum((emb - ref_emb)**2,axis=1).squeeze()
    #         keep = np.reshape(sims>thr,(M,M))
    #         keep[p_m>0] = 1
    #         pred_masks[i,1][keep] = 1.
    # if cfg.MRCNN.USE_CLS_EMBS:
    #     embs_logits = workspace.FetchBlob(
    #         core.ScopedName('mask_emb_logits')
    #     )
    #
    #     embs_logits = np.reshape(embs_logits,(-1,M,M,2))
    #     bs = embs_logits.shape[0]
    #     for i in range(bs):
    #         logits = embs_logits[i].transpose((2,0,1))
    #         probs = softmax(logits)
    #
    #         pred_masks[i,1][probs[1]>0.95] = 1.
...
def im_detect_keypoints(model, im_scale, boxes):
    ...
    if cfg.MRCNN.USE_KPS_CONV and cfg.KRCNN.BBOX_CASCADE_KPT_ON:
        '''
        inputs = {'mask_rois': _get_rois_blob(boxes, im_scale)}
        if cfg.FPN.MULTILEVEL_ROIS:
           _add_multilevel_rois_for_test(inputs, 'mask_rois')
        ...
        if cfg.MRCNN.CLS_SPECIFIC_MASK:
            pred_masks = pred_masks.reshape([-1, cfg.MODEL.NUM_CLASSES, 28, 28])
        else:
            pred_masks = pred_masks.reshape([-1, 1, M, M])
        for i in range(pred_heatmaps.shape[0]):
            p_mask = pred_masks[i][1]
            p_mask = cv2.resize(p_mask,(56,56))
            pred_heatmaps[i] = pred_heatmaps[i] * p_mask[np.newaxis]
    return pred_heatmaps
...
def box_results_with_nms_and_limit(scores, boxes):
    ...
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        ...
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            ...
            for j in range(1, num_classes):
                # cls_boxes_w = cls_boxes[j][:, 2] - cls_boxes[j][:, 0]
                # cls_boxes_h = cls_boxes[j][:, 3] - cls_boxes[j][:, 1]
                # keep_cond = np.logical_and(cls_boxes_w>30,cls_boxes_h>30)
                # keep_cond = np.logical_and(keep_cond,cls_boxes[j][:, -1] >= image_thresh)
...
def im_detect_body_uv(model, im_scale, boxes):
    ...
    if cfg.BODY_UV_RCNN.USE_CLS_EMBS or (cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON):
        person_mask = workspace.FetchBlob(core.ScopedName('person_mask')).squeeze()
    ...
    for ind, entry in enumerate(boxes):
        ...
        if cfg.BODY_UV_RCNN.USE_CLS_EMBS:
           CurPMask = np.swapaxes(person_mask[ind], 0, 2)
           CurPMask = cv2.resize(CurPMask, (by, bx))
           CurPMask = np.swapaxes(CurPMask, 0, 2)
           CurPMaskProb = softmax(CurPMask)
           CurPMask = np.argmax(CurPMask, axis=0)

           Index_mask = (CurPMaskProb[1]>cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)

        elif (cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON) or cfg.BODY_UV_RCNN.USE_AMA_NET:
           ...
           CurPMaskProb = softmax(CurPMask)
           CurPMask = np.argmax(CurPMask, axis=0)
           Index_mask = (CurPMask > 0).astype(np.float32)
           Index_mask = (CurPMaskProb[1]>cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)
        else:
            person_mask = workspace.FetchBlob(core.ScopedName('person_mask')).squeeze()
            if person_mask.ndim == 3:
            ...
            CurPMaskProb = softmax(CurPMask)
            CurPMask = np.argmax(CurPMask, axis=0)
            Index_mask = (CurPMask > cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)
...
def im_detect_aug_body_uv(model, im, boxes):
    uv_out = im_detect_body_aug(model, im, boxes)
    AnnIndex = uv_out[0]
    Index_UV = uv_out[1]
    U_uv = uv_out[2]
    V_uv = uv_out[3]
    if cfg.BODY_UV_RCNN.USE_CLS_EMBS or cfg.BODY_UV_RCNN.USE_AMA_NET or (
            cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON):
        person_mask = uv_out[4]
    outputs = []
    K = cfg.BODY_UV_RCNN.NUM_PATCHES + 1
    for ind, entry in enumerate(boxes):
        # Compute ref box width and height
...
def im_detect_uv(model, im_scale, boxes):
    """Compute body uv predictions."""
    M = cfg.BODY_UV_RCNN.HEATMAP_SIZE
    P = cfg.BODY_UV_RCNN.NUM_PATCHES
    if boxes.shape[0] == 0:
    ...
    K = cfg.BODY_UV_RCNN.NUM_PATCHES + 1
    outputs = []
    outputs.append(AnnIndex)
    outputs.append(Index_UV)
    outputs.append(U_uv)
    outputs.append(V_uv)

    if cfg.BODY_UV_RCNN.USE_CLS_EMBS or cfg.BODY_UV_RCNN.USE_AMA_NET or (cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON):
        person_mask = workspace.FetchBlob(core.ScopedName('person_mask')).squeeze()
        if person_mask.ndim == 3:
           person_mask = np.expand_dims(person_mask, axis=0)
        outputs.append(person_mask)
    return outputs

def im_detect_body_aug(model, im, boxes):
    """Performs mask detection with test-time augmentations.
    ...
    # Compute masks for the original image (identity transform)
    im_scale_i = im_conv_body_only(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE)
    uv_i = im_detect_uv(model, im_scale_i, boxes)
    uv_ts.append(uv_i)

    # Perform mask detection on the horizontally flipped image
    if cfg.TEST.UV_AUG.H_FLIP:
        uv_hf = im_detect_body_hflip(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes
        )
        uv_ts.append(uv_hf)
    #
    ...
    for scale in cfg.TEST.UV_AUG.SCALES:
        ...
        masks_scl = im_detect_body_scale(model, im, scale, max_size, boxes)
        uv_ts.append(masks_scl)

        if cfg.TEST.UV_AUG.SCALE_H_FLIP:
            masks_scl_hf = im_detect_body_scale(
                model, im, scale, max_size, boxes, hflip=True
            )
            uv_ts.append(masks_scl_hf)
    #


    # Combine the predicted soft masks
    if uv_ts.__len__()>0:
        num_output = len(uv_ts[0])
    else:
        return None
    for aug_num in range(1, len(uv_ts)):
        uv_ts[0][-1] += uv_ts[aug_num][-1]
    uv_ts[0][-1] /= len(uv_ts)
...
def im_detect_body_hflip(model, im, target_scale, target_max_size, boxes):
    """Performs mask detection on the horizontally flipped image.
    Function signature is the same as for im_detect_mask_aug.
    """
    # Compute the masks for the flipped image
    Index_Symmetry_List = [0, 1, 2, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15, 18, 17, 20, 19, 22, 21, 24,
                           23]
    im_hf = im[:, ::-1, :]
    boxes_hf = box_utils.flip_boxes(boxes, im.shape[1])

    im_scale = im_conv_body_only(model, im_hf, target_scale, target_max_size)
    uv_hf = im_detect_uv(model, im_scale, boxes_hf)

    # Invert the predicted soft masks
    new_I = np.zeros_like(uv_hf[1])
    new_I[:, 0] = uv_hf[1][:, 0].copy()
    for I in range(1, 25):
        new_I[:, I] = uv_hf[1][:, Index_Symmetry_List[I]]
    # dp part flip
    

    for i in range(len(uv_hf)):
        uv_hf[i] = uv_hf[i][:, :, :, ::-1]

    return uv_hf


def im_detect_body_scale(
    model, im, target_scale, target_max_size, boxes, hflip=False
):
    """Computes masks at the given scale."""
    if hflip:
        masks_scl = im_detect_body_hflip(
            model, im, target_scale, target_max_size, boxes
        )
    else:
        im_scale = im_conv_body_only(model, im, target_scale, target_max_size)
        masks_scl = im_detect_uv(model, im_scale, boxes)
    return masks_scl


def im_detect_body_aspect_ratio(model, im, aspect_ratio, boxes, hflip=False):
    """Computes mask detections at the given width-relative aspect ratio."""
